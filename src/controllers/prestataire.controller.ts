...
const checkAccess = async (weddingId: string, userId: string) => {
  return prisma.weddingCouple.findFirst({
...
export const addPrestataire = async (req: AuthRequest, res: Response) => {
  try {
    const { id: weddingId } = req.params
    const body = req.body as AddPrestataireInput
    ...
    let prestataire = await prisma.prestataire.findFirst({
      where: {
    ...
    const existing = await prisma.weddingPrestataire.findUnique({
      where: {
    ...
    const weddingPrestataire = await prisma.weddingPrestataire.create({
      data: {
        weddingId,
        prestataireId: prestataire.id,
        statut: 'CONTACTE',
    ...
    if (wedding) {
      ...
      if (rappels.length > 0) {
        await prisma.rappel.createMany({ data: rappels })
      }
    }
...
export const getPrestataires = async (req: AuthRequest, res: Response) => {
  try {
    const { id: weddingId } = req.params
    ...
    const prestataires = await prisma.weddingPrestataire.findMany({
      where: {
        weddingId,
        ...(statut && { statut: statut as any }),
        ...(categorie && {
          prestataire: { categorie: categorie as any },
        }),
      },
      include: { prestataire: true },
...
export const getPrestataire = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const wp = await prisma.weddingPrestataire.findUnique({
      where: { id: prestId },
      include: {
...
export const updatePrestataire = async (req: AuthRequest, res: Response) => {
  try {
    const { id: weddingId, prestId } = req.params
    const body = req.body as UpdatePrestataireInput
    ...
    const updated = await prisma.weddingPrestataire.update({
      where: { id: prestId },
      data: {
...
export const removePrestataire = async (req: AuthRequest, res: Response) => {
  try {
    const { id: weddingId, prestId } = req.params
    ...
    const wp = await prisma.weddingPrestataire.findUnique({ where: { id: prestId } })
    if (!wp || wp.weddingId !== weddingId) {
      return res.status(404).json({ success: false, error: 'Prestataire introuvable' })
    }

    // Supprimer les rappels liés + la liaison
    await prisma.rappel.deleteMany({ where: { weddingPrestataireId: prestId } })
    await prisma.weddingPrestataire.delete({ where: { id: prestId } })
...
export const evaluerPrestataire = async (req: AuthRequest, res: Response) => {
  try {
    const { id: weddingId, prestId } = req.params
    const { note, commentaire } = req.body as EvaluationInput
