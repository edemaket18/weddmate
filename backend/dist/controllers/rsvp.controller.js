...
exports.getRsvpInfo = exports.getRsvpStats = exports.deleteInvite = exports.updateInvite = exports.addInviteManuel = exports.getInvites = exports.submitRsvp = exports.getRsvpPage = void 0;
...
const getRsvpPage = async (req, res) => {
    try {
        ...
        const wedding = await prisma_1.prisma.wedding.findUnique({ where: { slug } });
        // Lire le template HTML
        const htmlPath = path_1.default.join(process.cwd(), 'public', 'rsvp.html');
...
exports.getRsvpPage = getRsvpPage;
//Soumettre RSVP
const submitRsvp = async (req, res) => {
    try {
        const { slug } = req.params;
        const { nom, prenom, statut, whatsapp, telephone, nombreAccompa, regimeAliment, transport, hebergement, cote, messageAuxMaries, } = req.body;
        ...
        if (statut === 'CONFIRME' && whatsapp) {
            ...
            if (j7 > new Date()) {
                await prisma_1.prisma.rappel.create({
                    data: {
                        weddingId: wedding.id,
                        type: 'RSVP_INVITE_J7',
                        canal: 'WHATSAPP',
        ...
        // Notifier les mariés
        const couples = await prisma_1.prisma.weddingCouple.findMany({ where: { weddingId: wedding.id } });
        if (couples.length > 0) {
...
const addInviteManuel = async (req, res) => {
    try {
        const { id: weddingId } = req.params;
        if (!await checkAccess(weddingId, req.user.id))
            return res.status(403).json({ success: false, error: 'Accès refusé' });
...
exports.addInviteManuel = addInviteManuel;
const updateInvite = async (req, res) => {
    try {
        const { id: weddingId, inviteId } = req.params;
        if (!await checkAccess(weddingId, req.user.id))
            return res.status(403).json({ success: false, error: 'Accès refusé' });
        const invite = await prisma_1.prisma.invite.findUnique({ where: { id: inviteId } });
        ...
        const updated = await prisma_1.prisma.invite.update({
            where: { id: inviteId },
            data: {
                ...(statut && { statut }),
...
const deleteInvite = async (req, res) => {
    try {
        ...
        if (!invite || invite.weddingId !== weddingId)
            return res.status(404).json({ success: false, error: 'Invité introuvable' });
        await prisma_1.prisma.invite.delete({ where: { id: inviteId } });
...
const getRsvpStats = async (req, res) => {
    try {
        ...
        if (!await checkAccess(weddingId, req.user.id))
            return res.status(403).json({ success: false, error: 'Accès refusé' });
        const invites = await prisma_1.prisma.invite.findMany({ where: { weddingId } });
...
exports.getRsvpStats = getRsvpStats;
exports.getRsvpInfo = exports.getRsvpPage;
/*
